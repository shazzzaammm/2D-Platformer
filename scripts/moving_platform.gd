...
func _ready():
	initial_position = $AnimatableBody2D.position
...
func start_tween():
	...
	tween.set_loops().set_parallel(false).set_trans(Tween.TRANS_LINEAR)
...
func _physics_process(_delta):
	if (initial_position.distance_to($AnimatableBody2D.position) < .5) and not enabled and tweening:
		stop_tween()
		$AnimatableBody2D.position = initial_position
